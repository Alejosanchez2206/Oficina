...
const apiUrl = 'http://localhost:5000/api/sedes';
...
btnAgregar.onclick = function () {
    document.getElementById('modalTitle').textContent = 'Nueva Sedes';
    form.reset();
...
form.onsubmit = async function (e) {
    ...
    try {
        let url = apiUrl;
        let method = 'POST';
        let successMessage = 'Sedes creada exitosamente';

        if (isEditMode) {
            url = `${apiUrl}/${editingCode}`;
            method = 'PUT';
            successMessage = 'Sedes actualizada exitosamente';
        }
...
function editarModulo(modulo) {
    ...
    document.getElementById('modalTitle').textContent = 'Editar Sedes';
...
async function eliminarModulo(codigo) {
    if (confirm('¿Está seguro de eliminar este Sedes?')) {
        try {
            ...
            if (response.ok) {
                await loadData();
                alert('Sedes eliminado exitosamente');
            } else {
